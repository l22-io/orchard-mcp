# orchard-mcp -- Product Requirements Document
...
- **AppleScript-based servers** (supermemoryai/apple-mcp, pyapple-mcp, applescript-mcp): Fragile, break on recurring events, require per-app Automation permissions that vary by caller, poor error handling.
- **Calendar-only EventKit servers** (apple-calendar-mcp by EgorKurito, che-ical-mcp, mcp-server-apple-events by FradSer): Solid Calendar (and sometimes Reminders) via native EventKit, but no Mail support.
- **Mail-only servers** (apple-mail-mcp, errol-mail): Either AppleScript-based (fragile) or read the Mail SQLite DB directly (requires Full Disk Access, brittle across macOS versions).
...
Existing servers and their limitations:

- **EgorKurito/apple-calendar-mcp**: TypeScript + Swift CLI. Calendar only. 6 tools. Good architecture (our reference). No Mail, no Reminders.
- **FradSer/mcp-server-apple-events**: TypeScript + Swift. Calendar + Reminders. Comprehensive prompts. No Mail. Active maintenance.
- **kiki830621/che-ical-mcp**: Pure Swift MCP server. 24 calendar/reminders tools. No Mail. Self-contained binary.
- **snarris/apple-eventkit-mcp**: Python + PyObjC. Calendar + Reminders. No Mail.
- **errol-mail**: Python. Reads Mail SQLite DB + .emlx files. Needs Full Disk Access. No Calendar.
- **supermemoryai/apple-mcp**: TypeScript (bun). All Apple apps via AppleScript. Broad but shallow. Recurring events break.
- **pyapple-mcp**: Python. All Apple apps via AppleScript. Same AppleScript limitations.

Our differentiation: **Calendar + Mail + Reminders in one server, native frameworks, production quality, npm-distributable.**
...
**No OAuth or web-based authentication required.** orchard-mcp reads from the local
...
flows with Google, Microsoft, iCloud, etc. natively. orchard-mcp never communicates
with any remote service.

The only permissions orchard-mcp needs are macOS TCC (Transparency, Consent, and Control) grants:
...
This is a key architectural advantage over competitors that use Google Calendar API or
...
### Interactive setup wizard (`orchard-mcp setup`)
...
- Package name: `@l22-io/orchard-mcp`
- Install: `npx @l22-io/orchard-mcp` or `npm install -g @l22-io/orchard-mcp`
...
Generated by `orchard-mcp setup`. Manual alternatives:
...
```json
{"mcpServers": {"orchard": {"command": "npx", "args": ["@l22-io/orchard-mcp"]}}}
```
...
- `orchard-mcp setup` interactive CLI wizard (prerequisite checks, binary acquisition, TCC permission triggers, MCP client config generation, validation)
